-- Fix email stitching for order_placed events that carry orphaned session_ids.
--
-- Problem (emerged ~May 27 2026): Shopify started sending a checkout-generated
-- session_id on server-side order_placed events instead of null. The previous
-- stitch condition only handled o.session_id IS NULL, so these new events were
-- never stitched to the visitor's browsing session.
--
-- Fix: Also stitch events whose session_id appears on no other event besides
-- order_placed itself — i.e. an "orphaned" Shopify checkout session.
...
begin
  ...
  -- Stitch server-side order events (null session_id OR orphaned checkout
  -- session_id) to the most recent prior browsing session by email.
  --
  -- An "orphaned" session is one where the session_id appears on no event
  -- other than order_placed — these are Shopify-generated checkout session_ids
  -- that have no corresponding page-view events in our analytics.
